...
#define SPI_CS_L() gpioPinWrite(GPIO_SPI_FLASH_CS, _DEF_LOW)
#define SPI_CS_H() gpioPinWrite(GPIO_SPI_FLASH_CS, _DEF_HIGH)
...
bool spiFlashInit(void)
{
  ...
  {
    if (id_tbl[1] == 0x1C && id_tbl[2] == 0x71 && id_tbl[3] == 0x18)
    {
      logPrintf("[OK] spiFlashInit()\n");
      logPrintf("     EN25QX128A Found\r\n");
      ret = true;
    }
    else
    {
      logPrintf("[NG] spiFlashInit()\n");
      logPrintf("     EN25QX128A Not Found %X %X %X\r\n", id_tbl[1], id_tbl[2], id_tbl[3]);
      ret = false;
    }
  }
  else
